...
resource "azurerm_kubernetes_cluster" "capstone" {
  ...
  api_server_access_profile {
    authorized_ip_ranges = ["${var.workstation_IP}", "${azurerm_public_ip.aks_public_ip.ip_address}/32"]
  }
  ...
  depends_on = [azuread_group.aks_admins, azurerm_subnet_nat_gateway_association.aks_natgw_association, azurerm_nat_gateway_public_ip_association.aks_natgw_association] #, azurerm_log_analytics_workspace.aks]
